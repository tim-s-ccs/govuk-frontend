...
When making changes to the codebase that could affect our users, we want to minimise the number of breaking changes in a major version that cannot be made before updating to the major version.

## Deprecating and removing features

Wherever possible, deprecate features and then remove them in the next major release.
...
Features should not be deprecated whilst they are relied on by other parts of GOV.UK Frontend.

### Tell users about the deprecation

Tell users about the deprecation in the release notes.

Don't assume that users will understand the term 'deprecation'. Wherever possible, explain why we are deprecating the feature and what we want users to do if they are currently using it.
...
### Deprecating an parameter for a Sass mixin or function

If possible, update the mixin or function to maintain the existing functionality whilst outputting a warning.
...
When renaming things, keep the old name available as an alias and mark it as deprecated.
...
///
/// @param {String} $spline Spline to reticulate
/// @param {String} $spilne Deprecated. Use $spilne instead.
